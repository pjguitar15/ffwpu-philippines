...
export function NewsForm({
  ...
  type PendingItem = { id: string; pct: number; error?: string | null }
  const [pendingGallery, setPendingGallery] = useState<PendingItem[]>([])
  ...
  async function uploadGalleryFiles(fileList: FileList | File[]) {
    ...
    // respect max items (12)
    const remaining =
      MAX_GALLERY_ITEMS - (values.gallery?.length || 0) - pendingGallery.length
    if (remaining <= 0) {
      toast({ title: 'Gallery is full', description: 'Max 12 images.' })
      return
    }
    ...
    const ids = toUpload.map(() => uid())
    setPendingGallery((prev) => [
      ...prev,
      ...ids.map((id) => ({ id, pct: 0, error: null })),
    ])
        ...
        (file, idx) =>
          new Promise<void>((resolve) => {
            ...
            xhr.onload = () => {
              try {
                ...
                if (xhr.status >= 200 && xhr.status < 300 && data?.url) {
                  // commit to gallery list
                  setValues((v) => ({
                    ...v,
                    gallery: [...(v.gallery || []), data.url].slice(
                      0,
                      MAX_GALLERY_ITEMS,
                    ),
                  }))
            ...
            xhr.onerror = () => {
              setPendingGallery((prev) =>
                prev.map((p) =>
                  p.id === id ? { ...p, error: 'Network error' } : p,
                ),
              )
              resolve()
            }
          ...
          >
            ...
            <div className='flex-1 min-h-0 overflow-y-auto px-6 pb-6 pr-7'>
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                ...
                {/* Row 3: Gallery uploader (modern UI) */}
                <div className='md:col-span-3'>
                  <div className='flex items-center justify-between mb-1.5'>
                    <label className='text-sm font-medium flex items-center gap-2'>
                      <Images className='h-4 w-4 text-sky-600 dark:text-sky-300' />
                      Gallery images
                      <span className='text-xs text-muted-foreground ml-2'>
                        {(values.gallery?.length || 0) + pendingGallery.length}/
                        {MAX_GALLERY_ITEMS}
                      </span>
                    </label>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          if (files?.length) await uploadGalleryFiles(files)
                  ...
                  <p className='text-xs text-muted-foreground mb-2'>
                    Drag & drop or click “Add images.” You can upload up to 12
                    images.
                  </p>

                  {/* Dropzone */}
                  <div
                    className='relative rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 hover:border-sky-400 transition-colors p-4'
                    ...
                    {galleryOpen && (
                      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3'>
                        ...
                        {(values.gallery || []).map((url, idx) => (
                          <div
                            key={`${url}-${idx}`}
                            className='relative overflow-hidden rounded-lg ring-1 ring-black/10 shadow bg-white'
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={url}
                            ...
                            <button
                              type='button'
                              className='absolute top-2 right-2 rounded-full bg-white/90 hover:bg-white px-2 py-1 text-xs text-rose-600 shadow ring-1 ring-black/10'
                              onClick={() =>
                                setValues((prev) => ({
                                  ...prev,
                        ...
                        {pendingGallery.map((p) => (
                          <div
                            key={p.id}
                            className='relative overflow-hidden rounded-lg ring-1 ring-black/10 shadow bg-slate-100 dark:bg-slate-900'
                          >
                            ...
                            <div className='absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/30'>
                              {!p.error ? (
                                <>
                                  ...
                                  <Progress className='w-2/3' value={p.pct} />
