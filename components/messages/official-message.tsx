    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-100 flex flex-col">
      ...
      <section className="w-full py-16 px-4 bg-gradient-to-b from-slate-900 to-slate-800 text-white text-center">
        <div className="max-w-3xl mx-auto">
          <div className="mb-4 text-xs font-semibold tracking-wide uppercase opacity-80">STATEMENTS & MESSAGES</div>
          <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4 leading-tight">Message from the {position || title}</h1>
        ...
        <div className="max-w-3xl w-full mx-auto">
          <div className="flex flex-col md:flex-row items-center gap-8 mb-8">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-base text-slate-700 font-medium mb-1">{title}</p>
              <p className="text-sm text-slate-500">Serving with vision, integrity, and dedication</p>
            </div>
          </div>
          <div className="prose prose-lg max-w-none text-slate-900 mb-10 font-serif">
            {message
              .filter(Boolean)
              .map((paragraph, idx) => (
                <p key={idx} className="mb-6 last:mb-0 whitespace-pre-line">{paragraph}</p>
              ))}
          </div>
          <div className="bg-slate-50 rounded-lg p-6 shadow-inner">
            <h3 className="text-lg font-semibold text-blue-800 mb-2">About {name}</h3>
